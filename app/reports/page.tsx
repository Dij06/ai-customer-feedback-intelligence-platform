...
import { FileBarChart, Printer, Trash2 } from 'lucide-react';
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (data.success && data.report) {
        ...
        toast.success('VoC report generated successfully!');
      } else {
        toast.error(data.error || 'Failed to generate report');
      }
    } catch (err) {
      console.error('Error generating report:', err);
      toast.error('Error generating executive report');
    } finally {
      setGenerating(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200/60 dark:border-slate-800/60 pb-5 sm:pb-6 print:hidden">
          ...
          <div className="flex items-center gap-2 flex-wrap shrink-0">
            ...
            {selectedReport && userRole !== 'VIEWER' && (
              <button
                onClick={() => setReportToDelete(selectedReport.id)}
                disabled={deletingId === selectedReport.id}
                className="inline-flex items-center gap-1.5 h-9 px-3.5 text-xs font-semibold text-rose-600 dark:text-rose-400 bg-white dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/50 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-colors shadow-2xs disabled:opacity-40"
                ...
                {reports.map((rep) => {
                  ...
                  return (
                    <div
                      ...
                      tabIndex={0}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') setSelectedReport(rep);
                      }}
                      className={`group relative w-full text-left p-3.5 rounded-xl border transition-all cursor-pointer ${
                        isSelected
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {userRole !== 'VIEWER' && (
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 p-1 -mr-1 -mt-0.5 rounded text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-all shrink-0"
                            title="Delete this report"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {userRole !== 'VIEWER' && (
                        <button
                          type="button"
                          onClick={() => setReportToDelete(selectedReport.id)}
                          className="print:hidden inline-flex items-center gap-1 text-xs text-rose-500 hover:text-rose-600 dark:text-rose-400 dark:hover:text-rose-300 font-medium px-2 py-1 rounded hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors"
                          title="Delete this report"
                        >
      ...
      {reportToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="relative w-full max-w-md rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-full bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400">
            ...
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-300">
              This will remove the generated executive summary and insights from your workspace. This action cannot be undone.
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                disabled={deletingId !== null}
                className="px-4 py-2 rounded-xl text-xs font-semibold text-white bg-rose-600 hover:bg-rose-700 transition-colors shadow-xs flex items-center gap-2"
              >
                {deletingId !== null ? (
                  <>
